...
toDoList.addEventListener('click', (e) => {
  ...
  if (dlt.includes(e.target)) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      //* /[\n\r]+|[\s]{2,}/g
      // [\n\r]+: 하나 이상의 줄바꿈 문자(\n 또는 \r)를 찾기 - 텍스트에서 모든 줄바꿈을 찾아내기 위해 사용
      // [\s]{2,}: 두 개 이상의 공백 문자를 찾기 - 문자열 내의 연속된 공백들을 정리하기 위해 사용
      // |: 논리적 '또는'을 의미하여, 줄바꿈 또는 연속된 공백 중 하나를 찾아냄
      // g: 전역 검색을 의미하여, 문자열 전체에서 모든 일치 항목을 찾음

      //* .replace(/[\n\r]+|[\s]{2,}/g, ' ')
      // : 정규 표현식에 해당하는 모든 부분을 단일 공백으로 교체
